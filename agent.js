...
// that routes across 40+ upstream providers with a built-in 3-tier fallback chain.
// Start it on your VPS with `9router start` (dashboard + API on :20128).
...
const DEFAULT_MODEL = process.env.LLM_MODEL || "glm/glm-4.7";
...
export async function agentLoop(goal, maxSteps = config.llm.maxSteps, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      // 9Router already does upstream provider fallback internally, so model-switching
      // here is opt-in via LLM_FALLBACK_MODEL (e.g. a known-good 9Router combo/model).
      const FALLBACK_MODEL = process.env.LLM_FALLBACK_MODEL || null;
      let response;
